...
importclass : 'import' imprtedClass = CLASSNAME ;
classDef : 'class' className = CLASSNAME ('(' CLASSNAME (',' CLASSNAME)* ')')? '{' class_body* '}';
class_body : varDec | methodDec | constructor | arrayDec ;
varDec : type = (TYPE | CLASSNAME) name = ID ;
arrayDec : type = (TYPE | CLASSNAME) '['INTEGER']' name = ID ;
methodDec : 'def'  (type = (TYPE | CLASSNAME)|'void') name = ID '(' parameter* ')''{' ( statement)* '}';
constructor : 'def' type = (TYPE | CLASSNAME) '(' parameter* ')''{' ( statement)* '}' ;
